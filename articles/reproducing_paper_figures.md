...
songR is validated against the original Python implementation
(Senanayake et al.) with a tiered set of golden-fixture tests
(`tests/testthat/test-reference-parity.R`). The honest, layer-by-layer
answer:
...
| Default visualization (UMAP-dispersed) | very close layout | MNIST Procrustes R^2 \approx 0.85 |
